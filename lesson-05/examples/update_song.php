<?php
  ...
  // validate that a song was selected
  if ( empty( $song_id ) ) {
    // concatenate an error message
    $error_msg .= "You must select a song.<br>";
  ...
  if ( $validated == false ) {
    ...
    $_SESSION['fail'] = "The song could not be updated:<br>{$error_msg}";
  ...
  $sql = 'UPDATE songs SET artist_id = :artist_id, title = :title, length = :length WHERE id = :id';
  ...
  // set our session variable with the error message
  $_SESSION['success'] = "You have updated the song, {$title}, successfully.";
