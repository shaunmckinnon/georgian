<?php
  ...
  // build the SQL
  $sql = 'SELECT categories.name as category, categories.id as category_id, products.name as name, products.description as description, products.price as price FROM products JOIN categories ON products.category_id = categories.id';
  $sth = $dbh->prepare( $sql );
  $sth->execute();
          ...
          <tbody>
            <?php foreach ( $result as $row ): ?>
              <tr>
                ...
                <td><?= $row['price'] ?></td>
                <td><a href="category_products.php?id=<?= $row['category_id'] ?>"><?= $row['category'] ?></a></td>
              </tr>
            <?php endforeach ?>
          </tbody>
        </table>
